%T2= readmatrix('EURGBP_24H0102-06302020.xlsx') ;
T=readmatrix('EURGBP_24H01022020-11082023.xlsx') ;

%% nuisance
y=flip(log(T(:,3)));
rows_with_zeros = all(y == -inf, 2);
y(rows_with_zeros, :) = [];
...
D_sym=cov*[d2log_likelihood_dalpha_domega;d2log_likelihood_dbeta_domega];
...
xlabel('Alpha');
ylabel('Beta');
legend('unadjusted','true var', 'adjusted','estimated ');
title({['90% Confidence Sigma Ellipse for Alpha and Beta:'];['adjustment:v(theta|estimated)']});
grid on;
hold off
...
ylabel('Beta');
legend('unadjusted','true var', 'adjusted','estimated ');
title({['90% Confidence Sigma Ellipse for Alpha and Beta:'];['adjustment:v(theta|true)']});
grid on;
hold off
